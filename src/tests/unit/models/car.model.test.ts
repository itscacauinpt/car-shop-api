...
describe('Testing Car Model', () => {
  ...
  describe('Endpoint POST/cars', () => {
    it('creating with success', async () => {
      expect(await carModel.create(carMock)).to.be.deep.equal(carMockId)
    });
  });

  describe('Endpoint GET/cars', () => {
    it('finding all with success', async () => {
      expect(await carModel.read()).to.be.deep.equal([carMockId])
    });
  });

  describe('Endpoint GET/cars/:id', () => {
    it('finding one with success', async () => {
      expect(await carModel.readOne(carMockId._id)).to.be.deep.equal(carMockId)
    });
  ...
  describe('Endpoint PUT/cars/:id', () => {
    it('updating one with success', async () => {
      expect(await carModel.update(carMockId._id, carMockToUp)).to.be.deep.equal(carMockUpdated)
    });
    it('_id not found', async () => {
			try {
  ...
  describe('Endpoint DELETE/cars/:id', () => {
    it('deleting one with success', async () => {
      expect(await carModel.delete(carMockId._id)).to.be.deep.equal(carMockId)
    });
    it('_id not found', async () => {
			try {
